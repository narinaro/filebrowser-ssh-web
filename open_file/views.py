...
# Get login credentials (session variable)
    ...
    if request.GET.get('file', ''):
        ...
        client.close()
        return render(request, 'openFile.html', {'content': response_text,
                                                 'path': file,
                                                 'folder': request.GET.get('folder', '')})
    ...
    # No File specified
        ...
        if request.POST.get('content', ''):
            command = 'echo "{}" > {}'.format(request.POST.get('content', ''),
                                            request.POST.get('path', ''))
            stdin, stdout, stderr = client.exec_command(command)
            stdin.write(passwordRequest)
            client.close()
            link = 'http://sshide.de/filebrowser/?folder={}'.format(request.POST.get('folder', ''))
